...
impl RenderContextManager {
    pub fn new(display_id: &ObjectId) -> Result<Rc<Self>> {
        ...
        let not_current = unsafe {
            display.create_context(&config, &ContextAttributesBuilder::default().build(None))
        }
...
fn wayland_display_handle(display_id: &ObjectId) -> Result<RawDisplayHandle> {
    let ptr = NonNull::new(display_id.as_ptr().cast::<c_void>())
        .context("wl_display pointer was null")?;
...
fn wayland_surface_handle(surface_id: &ObjectId) -> Result<RawWindowHandle> {
    let ptr = NonNull::new(surface_id.as_ptr().cast::<c_void>())
        .context("wl_surface pointer was null")?;
